...
export function Marketplace() { //
  ...
  useEffect(() => {
    const fetchLiveContent = async () => {
      ...
      -- 2. Fetch live editable text strings
    ...
    <>
        ...
        <div className="mx-auto max-w-4xl px-4 py-20 text-center sm:px-6 sm:py-24"> {/* */}
          ...
          <div className="mx-auto mt-10 flex max-w-2xl flex-col gap-2 rounded-xl border border-border bg-card p-2 shadow-sm sm:flex-row"> {/* */}
            ...
            <div className="relative flex flex-1 items-center"> {/* */}
              ...
              <input id="search" type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by name, room type or location..." className="w-full rounded-md border border-border bg-background py-3 pl-9 pr-3 text-sm outline-none focus:border-primary" /> {/* */}
            </div> {/* */}
          </div> {/* */}
        </div> {/* */}
      </section> {/* */}
      ...
      <div className="border-b border-border bg-background"> {/* */}
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-3 px-4 py-6 sm:px-6 md:grid-cols-4"> {/* */}
          {CATEGORIES.map((c) => ( //
            <button key={c.id} type="button" onClick={() => jumpTo(c.id, c.target)} className="flex flex-col items-center gap-2 rounded-xl border border-border bg-secondary/50 p-5 transition-all hover:-translate-y-0.5 hover:border-primary hover:bg-card hover:shadow-sm"> {/* */}
              <c.icon className="size-7 text-primary" /> {/* */}
              <span className="text-sm font-semibold text-foreground">{c.label}</span> {/* */}
            </button> {/* */}
          ))} {/* */}
          <button type="button" onClick={() => scrollTo("services")} className="flex flex-col items-center gap-2 rounded-xl border border-border bg-secondary/50 p-5 transition-all hover:-translate-y-0.5 hover:border-primary hover:bg-card hover:shadow-sm"> {/* */}
        ...
        <section id="houses" className="scroll-mt-16 py-16"> {/* */}
          <div className="mx-auto max-w-6xl px-4 sm:px-6"> {/* */}
            ...
            {tabHouses.length > 0 ? ( //
              ...
              <p className="mt-8 rounded-xl border border-dashed border-border p-8 text-center text-muted-foreground">No matching listings in this category yet.</p> {/* */}
            )} {/* */}
          </div> {/* */}
        </section> {/* */}
      )} {/* */}
        ...
        <section id="items" className="scroll-mt-16 border-y border-border bg-secondary/40 py-16"> {/* */}
          <div className="mx-auto max-w-6xl px-4 sm:px-6"> {/* */}
            ...
            {loadingItems ? ( //
              ...
              <p className="mt-8 rounded-xl border border-dashed border-border p-8 text-center text-muted-foreground">No items match your search.</p> {/* */}
            )} {/* */}
          </div> {/* */}
        </section> {/* */}
      )} {/* */}
        ...
        <section id="laptops" className="scroll-mt-16 py-16"> {/* */}
          <div className="mx-auto max-w-6xl px-4 sm:px-6"> {/* */}
            ...
            {filteredLaptops.length > 0 ? ( //
              ...
              <p className="mt-8 rounded-xl border border-dashed border-border p-8 text-center text-muted-foreground">No laptops match your search.</p> {/* */}
            )} {/* */}
          </div> {/* */}
        </section> {/* */}
      )} {/* */}
